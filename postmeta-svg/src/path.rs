...
pub fn path_to_d(path: &BezierPath, precision: usize) -> String {
    ...
    d.push('M');
    write_point(&mut d, p0.x, flip_y(p0.y), precision);

    for seg in path.segments() {
        d.push('C');
        write_point(&mut d, seg.p1.x, flip_y(seg.p1.y), precision);
        d.push(' ');
        write_point(&mut d, seg.p2.x, flip_y(seg.p2.y), precision);
        d.push(' ');
        write_point(&mut d, seg.p3.x, flip_y(seg.p3.y), precision);
    }
